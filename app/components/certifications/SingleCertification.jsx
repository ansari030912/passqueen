...
const SingleCertification = ({ vendorData }) => {
  const [searchTerm, setSearchTerm] = useState("");

  const filteredData = vendorData?.vendor_certs?.filter((vendor) =>
    vendor?.cert_title.toLowerCase().includes(searchTerm.toLowerCase())
  );
          ...
          {filteredData?.map((item, i) => {
            return (
              <Grid item xs={12} lg={6} key={i} className="w-full relative">
                <Link
                  href={`/vendor-exam-training/${vendorData?.vendor_perma}/${item.cert_perma}`}
                >
